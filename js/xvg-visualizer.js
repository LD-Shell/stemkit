document.addEventListener('DOMContentLoaded', () => {
    ...
    // Built-in sample: a synthetic GROMACS RMSD .xvg with proper @ metadata.
    ...
    if (sampleXvgBtn) sampleXvgBtn.addEventListener('click', (e) => {
        ...
        for (let t = 0; t <= 1000; t += 10) {
            const rmsd = (0.12 + 0.08 * (1 - Math.exp(-t / 200)) + (Math.random() - 0.5) * 0.02).toFixed(4);
            const rg = (1.85 + 0.05 * Math.sin(t / 120) + (Math.random() - 0.5) * 0.01).toFixed(4);
            xvg += `${t.toFixed(1)}   ${rmsd}   ${rg}\n`;
        }
        plotLoader.classList.remove('hidden'); plotLoader.classList.add('flex');
        uploadZone.classList.add('opacity-0', 'pointer-events-none');
        workspace.classList.remove('hidden');
        parseDataBuffer(xvg, 'sample_rmsd.xvg');
        plotLoader.classList.remove('flex'); plotLoader.classList.add('hidden');
    });
    ...
    function genXvgCode() {
        ...
        const xi = state.xIndex;
        const yIdx = [...state.activeYIndices];
        const showMarkers = plotMarkers.checked;
        const logY = plotLogY.checked;

        let c = `import matplotlib.pyplot as plt\nimport numpy as np\n\n`;
        c += `# --- Load your .xvg (skips GROMACS @/# metadata lines) ---\n`;
        c += `data = np.loadtxt('your_file.xvg', comments=['@', '#'])\n`;
        c += `# Columns: 0 = ${state.headers[xi] || 'x'}${yIdx.map(i => `, ${i} = ${state.headers[i] || 'y' + i}`).join('')}\n\n`;
        c += `x = data[:, ${xi}]\n\n`;
        c += `fig, ax = plt.subplots(figsize=(8, 5), dpi=150)\n`;
        const palette = ['#2563eb', '#ef4444', '#10b981', '#f59e0b', '#8b5cf6', '#06b6d4', '#ec4899'];
        yIdx.forEach(i => {
            const color = palette[i % palette.length];
            const label = state.headers[i] || `Dataset ${i}`;
            const style = showMarkers ? `, marker='o', markersize=3` : '';
            c += `ax.plot(x, data[:, ${i}], color='${color}', lw=1.5${style}, label=${py(label)})\n`;
        });
        c += `\nax.set_xlabel(${py(state.headers[xi] || state.xAxisLabel || 'x')})\n`;
        c += `ax.set_ylabel(${py(state.yAxisLabel || 'y')})\n`;
        if (state.title) c += `ax.set_title(${py(state.title)})\n`;
    ...
    if (xvgPyBtn) xvgPyBtn.addEventListener('click', () => {
        xvgCodeBlock.textContent = genXvgCode();
