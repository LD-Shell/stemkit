...
Archived release \texttt{v0.1.1}:
\url{https://doi.org/10.5281/zenodo.21544868} (the concept DOI
\texttt{10.5281/zenodo.21543112} resolves to the current release) \\
\hline
...
analysis is to be fixed, version-pinned and re-run. Every tool that
...
module mirrors, SciPy~\cite{virtanen2020} and NumPy~\cite{harris2020} for
validation, the
primary statistical
literature~\cite{welch1947,dagostino1973,brown1974,holm1979,iglewicz1993,grubbs1969}
...
\node[box, right=of browser] (node) {\textbf{Node.js host}\\[1pt]
  ...
  \footnotesize 1128-test Jest suite; smoke test\\
...
\texttt{statistics} & Inferential statistics & 84 & jStat \\
...
Structure inspector & \texttt{structure}, \texttt{selection} & Geometry, mass breakdown, atom selection, 3D view \\
...
Statistics calculator & \texttt{statistics} & t-tests, ANOVA, correlation, non-parametric tests, assumption checks \\
...
Plot builder & \texttt{statistics} & Publication figures with matplotlib script export \\
...
LaTeX table builder & \texttt{latex} & LaTeX and Markdown tables with correct escaping \\
Equation editor & \texttt{latex} & Formula entry and LaTeX output \\
\bottomrule
\end{tabular}
\end{table}
...
test~\cite{brown1974}, chosen for its greater robustness to non-normality. Where
an assumption fails, a non-parametric alternative is recommended. Table~\ref{tab:methods} maps each computed quantity
...
Student $t$, $\chi^2$ and normal tails. Beyond $|z| = 8$ the vendored
...
statistic is checked against SciPy or NumPy rather than against the author's own
...
\newblock STEMKit: A client-side toolkit for computational chemistry and
  scientific data analysis (v0.1.1), Zenodo (2026).
