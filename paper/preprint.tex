...
Structure inspector & \texttt{structure}, \texttt{selection} & Geometry, mass breakdown, atom selection and search by residue or species, simulation box, 3D view \\
Coordinate manipulator & \texttt{structure} & Translate, rotate, re-box; PDB/GRO/XYZ interconversion \\
MD workflow generator & \texttt{slurm}, \texttt{scheduler}, \texttt{plumed} & Batch scripts for GROMACS and LAMMPS on four schedulers; PLUMED input \\
...
Plot builder & \texttt{statistics} & Publication figures at a set print size and resolution, with matplotlib script export \\
Data cleaner & \texttt{data-cleaning} & Delimited-text repair, type inference, reshaping \\
Scientific converter & \texttt{units} & 64 units in ten categories, CODATA-sourced \\
...
DOI to BibTeX & \texttt{bibtex} & Crossref lookup with field filtering \\
...
Zenodo~\cite{stemkit2026}. The first archived release is contemporaneous with this manuscript, so
