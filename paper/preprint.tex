...
from the same data, so an interactive result can be reproduced offline
without the browser.
...
\node[box, right=of browser] (node) {\textbf{Node.js host}\\[1pt]
  ...
  \footnotesize 1284-test Jest suite; smoke test\\
...
\texttt{bibtex} & Reference deduplication, sanitising & 140 & bibtex-parse-js \\
\texttt{xvg-parser} & GROMACS/PLUMED trajectory data & 108 & none \\
\texttt{structure} & Molecular geometry, PDB/GRO/XYZ & 100 & none \\
\texttt{curve-fitting} & Least-squares regression & 81 & regression.js \\
...
\texttt{error-bars} & Group summaries, error bars & 60 & jStat \\
...
statement and 97.7\% line coverage of \texttt{src/core/}. Its governing
...
Data cleaner & \texttt{data-cleaning} & Missing values, duplicates, filtering and column transforms \\
Scientific converter & \texttt{units} & 64 units in ten categories plus temperature, CODATA-sourced \\
BibTeX sanitiser & \texttt{bibtex} & Field removal, page ranges, braces around capitals in titles \\
...
\newblock STEMKit: A client-side toolkit for computational chemistry and
  scientific data analysis (v0.2.1), Zenodo (2026).
